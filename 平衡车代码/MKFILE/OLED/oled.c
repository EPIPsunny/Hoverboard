...
//修改管脚时，应修改本文件中的 OLED_Init 函数中管脚的配置和 oled.h 文件的端口配置
...
{      			    
	...
	for(t=0;t<16;t++)
	{
		OLED_WR_Byte(Hzk[2*no][t],OLED_DATA);				//如需更改字库  只需改变数组名Hzk即可
...
{ 	  
  ...
  GPIO_SetBits(GPIOB,GPIO_Pin_0|GPIO_Pin_11);	
	GPIO_InitStructure2.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_5|GPIO_Pin_7;	 
