...
u8 USART_RX_BUF1[USART1_REC_LEN];     //接收缓冲,最大USART_REC_LEN个字节.
u8  USART_TX_BUF1[USART1_SEND_LEN]; 			//发送缓冲,最大USART3_MAX_SEND_LEN字节
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
		...
		if((USART_RX_STA1&0x8000)==0)//接收未完成
			...
			if(USART_RX_STA1&0x4000)//接收到了0x0d
				...
				if(Res==0x0d)USART_RX_STA1|=0x4000;
					...
					USART_RX_STA1++;
					if(USART_RX_STA1>(USART1_REC_LEN-1))USART_RX_STA1=0;//接收数据错误,重新开始接收	  
					}		 
				}
			}   		 
     } 
